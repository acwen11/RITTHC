...
/**
This class describes a system of %units, by storing base %units for time, length and mass,
and computes derived %units.
The unit system in which to specify the new %units is not fixed, so that this class may be
used to convert between any two systems of %units.
The unit object can also be used to absolutely specify a unit system.
In that case, the convention is that the %units are expressed in SI %units.
There are predefined unit objects which express different geometric unit systems (and SI units)
in terms of the SI %units.
*/
class units {
protected:
  double ulength,utime,umass;
public:
  units() {*this = si();}
...
///Compute geometric units where length unit is meter
